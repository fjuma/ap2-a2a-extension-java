...
import io.ap2.a2a.extension.roles.utils.MessageUtils;
...
public class Tools {
    ...
    public void handleSearchPaymentMethods(
        ...
        List<PaymentMethodData> merchantMethodDataList = methodData.stream()
                .map(data -> new PaymentMethodData(, Map.of(PAYMENT_METHOD_DATA_DATA_KEY, data)))
     ...
     */
    public CompletableFuture<Void> handleSignedPaymentMandate(
            List<DataPart> dataParts,
            TaskUpdater updater,
            Task currentTask) throws AP2Exception {
